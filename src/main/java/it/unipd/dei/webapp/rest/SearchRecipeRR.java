 ...
 * @since 1.00
 */
public final class SearchRecipeRR extends AbstractRR {

    private final String CacheTag = "SearchRecipeRR";
    // 1 minute (poll on client is every 5 sec)
    private final long expirationTime = 60000;

    public SearchRecipeRR(final HttpServletRequest req, final HttpServletResponse res, Connection con) {
        ...
        try {
            ...
            if (cachedResult != null) {
                ...
                if (rec.size() >= size) {
                    ...
                    rec = new GetRecipesFromIngTagDAO(con, ingredients, tags, size).access().getOutputParam();
                    cache.put(searchCacheTag, new CacheEntry<>(rec, System.currentTimeMillis(), expirationTime));
                    session.setAttribute("cache", cache);

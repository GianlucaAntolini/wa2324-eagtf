...
document.getElementById("recoverButton").addEventListener("click", recoverRecipeList);
document.getElementById("removeButton").addEventListener("click", removeRecipeList);

// set click action for the two buttons in the context menu
document.getElementById("cmRecoverButton").addEventListener("click", recoverRecipeList);
document.getElementById("cmRemoveButton").addEventListener("click", removeRecipeList);

// recover checked recipes
function recoverRecipeList() {

	let msg = document.getElementById('confirmationMsg');
	if (msg != null)
		document.getElementById('confirmationMsg').remove();

	const recipeRecoveryList = [];
	...
	for (let i = 0; i < tmp.length; i++) {
		...
		xhr.onreadystatechange = function () {
			processResponse(this, tmp, recipeRecoveryList);
		};

		xhr.open("POST", "http://localhost:8080/dinner-dilemma-1.00/rest/recipe/" + tmp[i].parentNode.parentNode.querySelectorAll('td')[0].innerHTML + "/recover", true);
...
function removeRecipeList() {
	...
	for (let i = 0; i < tmp.length; i++) {
		...
		xhr.onreadystatechange = function () {
			processResponse(this, tmp, recipeRemovalList);
		};

		xhr.open("POST", "http://localhost:8080/dinner-dilemma-1.00/rest/recipe/" + tmp[i].parentNode.parentNode.querySelectorAll('td')[0].innerHTML + "/remove", true);
		xhr.setRequestHeader("Content-Type", "application/json");
		xhr.send();
	}
}
...
function processResponse(xhr, checkedList, confirmList) {
	...
	if (responseStatus == "recovered" || responseStatus == "removed") {
